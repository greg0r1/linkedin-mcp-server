...
import { LinkedInOAuthService } from './src/infrastructure/linkedin/oauth.service.js';
import { FileTokenStorage } from './src/infrastructure/storage/file-token-storage.js';
  ...
  try {
    ...
    try {
      ...
      if (companiesResponse.data.elements && companiesResponse.data.elements.length > 0) {
        ...
        companiesResponse.data.elements.forEach((element: any, index: number) => {
        ...
        const gdDevSolutions = companiesResponse.data.elements.find((element: any) => {
        ...
        if (gdDevSolutions) {
          const companyId = gdDevSolutions.organizationalTarget.split(':').pop();
          ...
          console.log('⚠️  GD Dev Solutions non trouvée dans vos pages administrées.');
          console.log('Vérifiez que vous êtes bien administrateur de cette page.');
        }
      } else {
      ...
      if (apiError.response?.status === 403) {
        console.error('❌ Accès refusé. Vous devez demander l\'accès au scope "r_organization_admin"');
        console.error('Allez sur LinkedIn Developer Portal > Products > Request access');
      } else {
